...
public partial class Index
 {
     ...
     private async Task LoadAndVerifyUser()
     {
         ...
         if (string.IsNullOrWhiteSpace(objectId) == false)
         {
             ...
             if (email.Equals(loggedInUser.ObjectIdentifier) == false)
             {
                 isDirty = true;
     ...
     private async Task FilterSuggestions()
     {
         ...
         if (!string.IsNullOrWhiteSpace(searchText))
         {
             ...
             {
                 //TODO - FIX
                 output = output.Where(s => (s.Suggestion ?? "").Contains(searchText, StringComparison.InvariantCultureIgnoreCase) || (s.Description ?? "").Contains(searchText, StringComparison.InvariantCultureIgnoreCase)).ToList() ?? new();
             }
     ...
     private async Task OrderByNew(bool orderByNew)
     {
         isSortedbyNew = orderByNew;
         await FilterSuggestions();
     }
     ...
     private async Task VoteUp(SuggestionModel suggestion)
     {
         if (loggedInUser is not null)
         {
             ...
             {
                 suggestions = suggestions.OrderByDescending(s => s.UserVotes.Count).ThenBy(s => s.DateCreated).ToList();
             }
         }
         else
         {
